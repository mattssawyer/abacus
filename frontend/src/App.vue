...
async function openPlaidLink(): Promise<void> {
  ...
  try {
    ...
    const handler = window.Plaid.create({
      ...
      onExit(linkError) {
        if (linkError) {
          error.value = 'Plaid Link closed with an error'
        }
      },
    })
...
<template>
  <main>
    ...
    <button :disabled="loading" @click="openPlaidLink">
      {{ loading ? 'Loading…' : 'Connect an account' }}
    </button>
    ...
    <section v-if="accounts.length">
      ...
      <ul>
        <li v-for="account in accounts" :key="account.account_id">
          ...
          <p>
            Current:
            {{ formatBalance(account.balances.current, account.balances.iso_currency_code) }}
          </p>
          <p v-if="account.balances.available !== null">
            Available:
            {{ formatBalance(account.balances.available, account.balances.iso_currency_code) }}
          </p>
        </li>
      </ul>
    </section>

    <p v-else-if="!loading">No accounts connected.</p>
  </main>
</template>
